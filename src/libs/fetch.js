...
export default class ChainFetch {
  ...
  async getTxs(hash, config = null) {
    ...
    if (ver && compareVersions(ver, '0.5') < 1) {
  ...
  async getStakingReward(address, config = null) {
    if (compareVersions(config ? config.sdk_version : this.config.sdk_version, '0.50') < 0) {
      return this.get(`/distribution/delegators/${address}/rewards`, config).then(data => commonProcess(data))
    }
    return this.get(`/lbm/distribution/v1/delegators/${address}/rewards`, config).then(data => commonProcess(data))
  }
